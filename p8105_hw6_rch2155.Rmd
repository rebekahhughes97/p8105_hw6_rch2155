...
The following code runs 5000 bootstrap samples and produces values for $\hat{r}^2$ and $\log(\hat{\beta}_0 * \hat{\beta}_1)$.
...
The first plot for $\hat{r}^2$ is fairly consistent except between 0.91 and 0.92 when there is a dip in the density. The first plot is centered around 0.91 it appears. The second plot for $\log(\hat{\beta}_0 * \hat{\beta}_1)$ is fairly consistent as well except for a slight dip in the density between 2.00 and 2.03. The second plot is centered around 2.00 approximately.
